...
import { faPlus,faEdit, faTimes,faMinus, faTrashAlt, faChevronLeft, faChevronRight ,faLeaf, faDrumstickBite } from "@fortawesome/free-solid-svg-icons";
import { useNavigate } from "react-router-dom";
import Loading from "../Components/Loading";
import Spinner from "../Components/Spinner";

const storeCity = "Indore";
...
function Cart() {
    ...
    const calculateDiscount = (res) => {
        res?.cartItems.forEach(ci => {
            if(ci.beverage!=null) return;
            ci.pizza.createdAt = new Date(ci.pizza.createdAt);
            // check if the pizza is older than 7 days 
            if (ci.pizza.createdAt.getTime() >= new Date().getTime() - 7 * 24 * 60 * 60 * 1000) {
                // apply 10% discount
                console.log("discount applied");
                ci.price = ci.price - (ci.price * 0.10);
                ci.discountPercent = 10;
            }
        });
        setCart(res);   
    }
                    ...
                    <>
                            ...
                            {cart.cartItems.map(item => (
                                ...
                                {/* <div className="col-span-12 lg:col-span-10 flex flex-col justify-between"> */}
                                    <div className="col-span-12 lg:col-span-2 img box">
                                        <img src={item?.pizza?.imageUrl} alt={item?.pizza?.name} className="max-lg:w-[78%] lg:w-[180px] rounded-lg" />
                                        <img src={item?.beverage?.image} alt={item?.beverage?.name} className="max-lg:w-[78%] lg:w-[180px] rounded-lg" />
                                    </div>
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <h6 className="text-indigo-600 font-manrope font-bold text-2xl leading-9 text-right">
                                                ...
                                                {item.discountPercent > 0 && (
                                                    <span> with {item.discountPercent}% off</span>
                                                )}
                                            </h6>
                        ...
                        {selectedCartItem && (
                            <EditCartItemModal
                                ...
                                isVegetarian={selectedCartItem.pizza.isVegetarian}
            ...
            <Modal
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg relative w-full max-w-md">
                    ...
                    <Pay
                        ...
                        cartId={cart.cartId}
                        orderPrice={cart.cartItems.reduce((acc, item) => acc + item.price, 0)}
                        isPickup={isPickup}
                        address={address}
